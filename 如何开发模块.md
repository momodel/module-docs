...
本文通过开发识别鸢尾花的类别的支持向量机模型来介绍如何使用调参工具包进行模型调参。

3.1 导入工具包

在`工具栏`中点击`插入代码工具`，弹出`工具列表栏`，找到`超参数调参工具`后点击`插入代码`。然后该工具的示例代码将自动插入到您的 `Notebook` 中，同时` mo `工具包将自动导入到你的项目中，你可在左侧的文件列表中找到` mo `文件夹。
...
打开 `Notebook` ，在 `cell` 输入`!pip install baytune`后`Shift + Enter`即可。
...
# 3.3.3 建立模型
model = svm.SVC(verbose=False, random_state=0, probability=True)
...
# 通过上述操作后，得到模型的结果为：
Train_score:0.8416666666666667
Test_score:0.6333333333333333
```
...
一般支持向量机分类模型（SVM.SVC）的超参数主要有以下三个：

- 惩罚系数`C`：用来平衡分类间隔`margin`和错分样本的，default C = 1.0；
- 核函数 `kernel`：参数选择有`RBF`, `Linear`,` Poly`, `Sigmoid`, 默认的是`RBF`;
- 核函数参数`gamma`：核函数的系数('Poly', 'RBF' and 'Sigmoid'), 默认是`gamma = 1 / n_features`。

3.4.2 定义调参的参数信息

`tunables`可选的参数类型为 `INT`、 `INT_EXP`、 `INT_CAT`、 `FLOAT`、 `FLOAT_EXP`、`FLOAT_CAT`、 `STRING`和`BOOL`。

- INT：整型；例如 [1, 10] 会从1到10的均匀分布中选择整型数值
- INT_EXP：指数型分布中选择整型数值；例如 [1, 1000] 会从1到1000的指数型分布（以10为底）中选择整型数值
- INT_CAT：枚举法，从已知列表中选择一个数据；例如 [1, 10, 1000] 会从列表的三个值中选择一个
- FLOAT： 浮点型, 例如 [1.0, 10.0] 会从1.0到10.0的均匀分布中选择浮点型数值
- FLOAT_EXP： 例如 [1.0, 1000.0] 会从1.0到1000.0的指数型分布（以10为底）中选择浮点型数值
- FLOAT_CAT： 例如 [1.0, 10.0, 100.0] 会从列表的三个值中选择一个
- STRING：字符串，例如 ["a", "b"，"c"] 会从列表的三个值中选择一个
- BOOL：布尔型
...
from btb import HyperParameter, ParamTypes
tunables = [
    ('kernel', HyperParameter(ParamTypes.STRING, ['linear', 'rbf', 'poly'])),
    ('C', HyperParameter(ParamTypes.INT, [1, 10])),
    ('gamma', HyperParameter(ParamTypes.FLOAT_CAT, [0.01, 0.1, 0.5, 1, 4]))
]
```

3.4.4 设置模型优化指标

整个调参的目的就是求优化目标的极值。本案例以准确率作为优化的目标。
...
tu = HyperparameterTuner(x_train, y_train, x_test, y_test, tunables, score_fun, model,trail_numer=30)
best_model, best_para = tu.start_tune()
print(tu.start_tune())
```

注意:
- 参数`trail_numer`，表示试验的次数，一般情况下设置大一点比较好，比如10次等。
- 因为调参可能需要花费较长的时间，建议创建 `job` 来进行模型的调参，并在 `job` 日志中查看调参的打印输出，更多关于 `job` 的信息，可参考[这里](http://www.momodel.cn:8899/docs/#/zh-cn/%E8%BE%B9%E5%AD%A6%E8%BE%B9%E5%81%9A?id=_4-%E5%88%9B%E5%BB%BA-gpu-job-%E8%AE%AD%E7%BB%83%E6%A8%A1%E5%9E%8B)。

3.4.7 获取模型的预测结果

```python
print("Train_score:{0}\nTest_score:{1}".format(best_model.score(x_train, y_train),best_model.score(x_test, y_test)))

# 经过优化后的运算结果为：
Train_score:0.8333333333333334
Test_score:0.7666666666666667
```
模型优化前后在训练集上的准确率变化不大，但是在测试集上的准确率得到13%左右的提升。
...
### 5. 查看 Job 运行进程
<img src='http://imgbed.momodel.cn/5ccea30ae3067c14d880bbba.jpg' width=50% height=50%>
<img src='http://imgbed.momodel.cn/5ccea30ae3067c14d880bbbb.jpg' width=50% height=50%>
...
点击左侧 Running Tab栏，点击 TensorBoard 图标按钮!<img src='http://imgbed.momodel.cn/5ccea30ee3067c14d880bbc1.jpg' width=10% height=10%>打开 TensorBoard 网页界面。

<img src='http://imgbed.momodel.cn/5ccea30be3067c14d880bbbc.jpg' width=50% height=50%>

查看你的模型 Graph 和训练情况。

<img src='http://imgbed.momodel.cn/5ccea311e3067c14d880bbc5.gif' width=90% height=90%>
...
<img src='http://imgbed.momodel.cn/5ccea30ee3067c14d880bbc2.jpg' width=50% height=50%>
